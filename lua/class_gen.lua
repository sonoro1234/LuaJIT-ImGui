...
local function make_function(method,def)
	...
	if def.nonUDT == 1 or next(def.defaults) then
		...
		--for k,v in pairs(def.defaults) do
			--special default treatment for ImStr
			if ArgumentType(def,k) == "ImStr" then
				--print("default for ImStr",v)
				table.insert(code,"    "..k.." = "..k.." or M.ImStr("..v..")")
			else
				if v == 'true' then
...
local function class_gen(sources)
	...
	cpp2ffi.table_do_sorted(structs, function(struct,funs)
		if struct ~= "ImGui" and struct ~= "ImVec2" and struct ~= "ImStr" and struct ~= "" then
